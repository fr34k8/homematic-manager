...
| D-41 | **(2026-09-09, maintainer) The Docker image keeps `HMM_ISSUE_COOKIE=true`** (OQ-15). A container never binds loopback, so without the cookie the UI would load and its socket would be refused on every page load; the image issues it so that the UI works out of the box, and the consequence - whoever reaches the published port is in - is said in two places: `docs/install-docker.md` names the three ways to lock it down, and the host **prints one warning line at start** whenever the cookie is issued on a non-loopback bind (the host itself never terminates TLS and cannot see a proxy in front of it, so it says this in every such position; `--no-issue-cookie` / `HMM_ISSUE_COOKIE=false` is what a setup with a proxy that issues the token uses, and that silences it). No image whose UI refuses until the page has been read. |
...
`build.yml`, click through the Electron artifact, tag, publish the draft
...
exists; OQ-14 (D-33) and, since 2026-09-09, OQ-15 (D-41) are decided; the install-from-the-published-artefacts round of
